...
% opts.fPath = 'U:\space_managed_data\BpodImager\Animals\mSM36\SpatialDisc\02-May-2017';
...
for iTrials = 1:length(trials)
    ...
    mov(:,Cnt(1) + (1:size(blueData,2))) = blueData;
...
for x = 1:length(ind)
    if x == length(ind)
        mov(:,ind(x)+1:end) = bsxfun(@minus, mov(:,ind(x)+1:end), blueAvg); % subtract blue mean 
        mov(:,ind(x)+1:end) = bsxfun(@rdivide, mov(:,ind(x)+1:end), blueAvg); % divide by blue mean 
    else
        mov(:,ind(x)+1:ind(x+1)) = bsxfun(@minus, mov(:,ind(x)+1:ind(x+1)), blueAvg); % subtract blue mean 
        mov(:,ind(x)+1:ind(x+1)) = bsxfun(@rdivide, mov(:,ind(x)+1:ind(x+1)), blueAvg); % divide blue mean 
    end
end
toc
...
toc;
disp('Saving V');
save([opts.fPath filesep 'firstV.mat'],'V','Sv','-v7.3');
toc;
...
for iTrials = 1:length(trials)
    ...
    blueV(:,Cnt + (1:size(blueData,3))) = U' * arrayShrink(blueData,mask,'merge');
