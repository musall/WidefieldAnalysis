function BpodImager_delayRegressModel_v2(cPath,Animal,Rec)
...
ridgeFolds = 20;    %folds for cross-validation when assessing predicted variance
...
for iTrials = 1:trialCnt
    ...
    %% choice and reward
    visRewardR{iTrials} = single((timeR .* (bhv.StimType(iTrials)  == 1)) * (-1 + (2 * bhv.Rewarded(iTrials)))); %visual trial, 1 for reward, -1 for no reward
    audRewardR{iTrials} = single((timeR .* (bhv.StimType(iTrials)  == 2)) * (-1 + (2 * bhv.Rewarded(iTrials)))); %audio trial, 1 for reward, -1 for no reward
    mixRewardR{iTrials} = single((timeR .* (bhv.StimType(iTrials)  == 3)) * (-1 + (2 * bhv.Rewarded(iTrials)))); %audio trial, 1 for reward, -1 for no reward
    
    choiceR{iTrials} = single(timeR .* bhv.ResponseSide(iTrials)); %choice regressor, takes -1 for left and 1 for right responses
    ...
    if iTrials == 1 %don't use first trial
        ...
        if ~SessionData.Rewarded(bTrials(iTrials)-1) %last trial was not rewarded
            if SessionData.DidNotChoose(bTrials(iTrials)-1) %no response in last trial
                ...
                prevRewardR{iTrials} = single(timeR .* -1);
            end
        else
            prevRewardR{iTrials} = single(timeR);
        end
        
%         %scale previous reward regressor with ITI. Impact of time on this is presumably not linear so there might be a better way of taking ITI into account.
%         if ~SessionData.DidNotChoose(bTrials(iTrials)-1)
%             lastStim = SessionData.RawEvents.Trial{bTrials(iTrials)-1}.Events.Wire3High; %time of stimulus onset in previous trial - measured from soundcard
%             ITI = (SessionData.TrialStartTime(bTrials(iTrials)) + stimTime(iTrials)) - (SessionData.TrialStartTime(bTrials(iTrials)-1) + lastStim); %inter-trial interval: time difference of previous to current stimulus onset.
%             prevRewardR{iTrials} = prevRewardR{iTrials} ./ ITI; 
%         end
    end
    ...
    if gaussShift > 1
        ...
        mixRewardR{iTrials} = mixRewardR{iTrials}(:,1:gaussShift:end);
    ...
    [times,Analog] = Widefield_LoadData([cPath 'Analog_'  num2str(trials(iTrials)) '.dat'],'Analog'); %load analog data    
    imgStart(iTrials) = times(1) * 86400;
...
fullR = [timeR lGrabR lGrabRelR rGrabR rGrabRelR lLickR rLickR leverInR lVisStimR rVisStimR lAudStimR rAudStimR visRewardR audRewardR ...
    mixRewardR prevRewardR choiceR prevChoiceR waterR piezoR whiskR pupilR vidR];
...
recLabels = {
    ...
    'lAudStim' 'rAudStim' 'visReward' 'audReward' 'mixReward' 'prevReward' 'choice' 'prevChoice' ...
...
recIdx = [
    ...
    ones(1,size(mixRewardR,2))*find(ismember(recLabels,'mixReward')) ...
...
clear stimR lGrabR lGrabRelR rGrabR rGrabRelR waterR lLickR rLickR leverInR ...
    lVisStimR rVisStimR lAudStimR rAudStimR visRewardR audRewardR mixRewardR prevRewardR choiceR ...
...
%     fakeR(:,cIdx) = [];
    ...
    [rankP(Cnt), rankH(Cnt)] = signrank(meanRsq, fullRsq, 'tail', 'left'); %check for significant reduction vs full model
