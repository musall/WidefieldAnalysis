function BpodImager_rebuildTrials(cMod,reload)
...
for iAnimals = 1:length(animals)
    if strcmpi(dataOverview{iAnimals,2},cMod)
        ...
        %% load data
        fPath = [cPath animals{iAnimals} filesep Paradigm filesep dataOverview{iAnimals,3} filesep];
        if isempty(dir([fPath 'Vc.mat'])) ||isempty(dir([fPath 'regData.mat'])) ||isempty(dir([fPath 'dimBeta.mat'])) || reload %check if file exists on hdd and pull from server otherwise
            if ~isdir(fPath); mkdir(fPath); end
            sourcePath = [sPath animals{iAnimals} filesep Paradigm filesep dataOverview{iAnimals,3} filesep];
            copyfile([sourcePath 'Vc.mat'],[fPath 'Vc.mat']);
            copyfile([sourcePath 'mask.mat'],[fPath 'mask.mat']);
            copyfile([sourcePath 'dimBeta.mat'],[fPath 'dimBeta.mat']);
            copyfile([sourcePath 'regData.mat'],[fPath 'regData.mat']);
        end
