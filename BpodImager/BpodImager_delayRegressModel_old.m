...
ridgeVal = 10000;     %initial estimate for ridge parameter
...
newRun = false;     %flag to indicate that all values should be recomputed (including ridge regression and correlation maps)
...
for iTrials = 1:trialCnt
    ...
    % time regressor, aligned to start of the (last) lever period. Zero lag regressor is the first frame after baseline.
    ...
    if offset > 0
        ...
        leverAlignR{iTrials}(:,1:end+offset) = timeR(:,1 - offset:(frames));
    end
    leverAlignR{iTrials}((preStimDur*sRate) + 1:end,:) = false; %should not extent into stimulus period, defined by current delay value

    stimAlignR{iTrials} = timeR(:,(preStimDur*sRate) + 1:end); %time regressor for postStim period.
...
recLabels = {
    ...
    'lAudStim' 'rAudStim' 'visRewardPreStim' 'visRewardPostStim' 'audRewardPreStim' 'audRewardPostStim' 'mixRewardPreStim' ...
    'mixRewardPostStim' 'visPrevRewardPreStim' 'visPrevRewardPostStim' 'audPrevRewardPreStim' 'audPrevRewardPostStim' ...
    'mixPrevRewardPreStim' 'mixPrevRewardPostStim' 'choicePreStim' 'choicePostStim' 'prevChoicePreStim' ...
    'prevChoicePostStim' 'water' 'pupil' 'pastVidR' 'presentVidR' 'futureVidR' 'piezoR'};
...
if exist([cPath 'dimCorrFull.mat'],'file') == 2
    if ~isunix
